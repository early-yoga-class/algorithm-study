...
매주 월요일 `09:02 (KST)`에 GitHub Actions가 실행되어 지난 1주 동안의 PR 제출 여부를 확인합니다.

## 동작 방식

1. `.github/workflows/pr-penalty-check.yml` 이 매주 월요일 `00:02 UTC`에 실행됩니다.
2. `scripts/check_pr_penalties.py` 가 `지난 월요일 09:02 ~ 이번 월요일 09:02 (KST)` 사이의 PR을 조회합니다.
...
- `cutoff_hour`, `cutoff_minute`: 마감 시각 조정
